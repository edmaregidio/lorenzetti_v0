...
bool CaloClusterConverter::convert( const CaloCluster *clus, CaloCluster_t &clus_t , cell_links_t &cell_links )
{
  if(clus){
      ...
      clus_t.rp2       = clus->rp2();
      clus_t.rp3       = clus->rp3();
      clus_t.rphad1       = clus->rphad1();
      clus_t.rphad2       = clus->rphad2();
      clus_t.rphad3       = clus->rphad3();
...
bool CaloClusterConverter::convert( const CaloCluster_t &cluster_t, CaloCluster *&cluster )
{
  ...
  cluster->setRp0(cluster_t.rp0);
  cluster->setRp1(cluster_t.rp1);
  cluster->setRp2(cluster_t.rp2);
  cluster->setRp3(cluster_t.rp3);
  cluster->setRphad1(cluster_t.rphad1);
  cluster->setRphad2(cluster_t.rphad2);
  cluster->setRphad3(cluster_t.rphad3);
