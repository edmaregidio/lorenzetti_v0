  ...
  Algorithm()
{
  ...
  declareProperty( "RpRingerKey"        , m_rpringerKey="RpRings"           );
...
StatusCode RootStreamNTUPLEMaker::bookHistograms( EventContext &ctx ) const
{
  ...
  std::vector<float>*rprings        = nullptr;
  ...
  outputTree->Branch("cluster_rp0"     , &rp0);
  outputTree->Branch("cluster_rp1"     , &rp1);
  outputTree->Branch("cluster_rp2"     , &rp2);
  outputTree->Branch("cluster_rp3"     , &rp3);
  outputTree->Branch("cluster_rphad1"     , &rphad1);
  outputTree->Branch("cluster_rphad2"     , &rphad2);
  outputTree->Branch("cluster_rphad3"     , &rphad3);
  outputTree->Branch("rings"          , &rings);
  outputTree->Branch("rprings"          , &rprings);
...
StatusCode RootStreamNTUPLEMaker::deserialize( int evt, EventContext &ctx ) const
{
  ...
  std::vector<xAOD::CaloRings_t         > *collection_rprings      = nullptr;
  ...
  InitBranch( tree, ("CaloRingsContainer_"         + m_rpringerKey).c_str(),  &collection_rprings);
  ...
  InitBranch( outputTree,"cluster_rp0"  , &rp0);
  InitBranch( outputTree,"cluster_rp1"  , &rp1);
  InitBranch( outputTree,"cluster_rp2"  , &rp2);
  InitBranch( outputTree,"cluster_rp3"  , &rp3);
  InitBranch( outputTree,"cluster_rphad1"  , &rphad1);
  InitBranch( outputTree,"cluster_rphad2"  , &rphad2);
  InitBranch( outputTree,"cluster_rphad3"  , &rphad3);
  

  { //main loop (from cluster to truth)
  
    InitBranch(outputTree, "rings", &rings);
    InitBranch(outputTree, "rprings", &rprings);

    SG::WriteHandle<xAOD::CaloRingsContainer> ringerContainer(m_ringerKey, ctx);
    SG::WriteHandle<xAOD::CaloRingsContainer> rpringerContainer(m_rpringerKey, ctx);

    ringerContainer.record( std::unique_ptr<xAOD::CaloRingsContainer>(new xAOD::CaloRingsContainer()));
    rpringerContainer.record( std::unique_ptr<xAOD::CaloRingsContainer>(new xAOD::CaloRingsContainer()));
    ...
    for (auto &cluster_t: *collection_cluster){
      for (auto &ringer_t : *collection_rings ){
        if (ringer_t.cluster_link == cluster_link){
          ...
          rp2     = cluster->rp2();
          rp3     = cluster->rp3();
          rphad1  = cluster->rphad1();
          rphad2  = cluster->rphad2();
          rphad3  = cluster->rphad3();
          
        }
        break;
      }
      for (auto &ringer_t: *collection_rprings){
        if (ringer_t.cluster_link == cluster_link){
          xAOD::CaloRings *rpringer = nullptr;
          xAOD::CaloRingsConverter cnv;
          cnv.convert(ringer_t, rpringer); // alloc memory
          for (auto rpring : rpringer->rings()) rprings->push_back(rpring);
        }
        break;
      }
