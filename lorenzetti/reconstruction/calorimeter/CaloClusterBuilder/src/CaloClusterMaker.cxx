...
StatusCode CaloClusterMaker::bookHistograms( SG::EventContext &ctx ) const
{
  ...
  store->add( new TH1F("cl_rp0", ";Count;R_{p} PS;"      , 100, 0.0  , 1.00) );
  store->add( new TH1F("cl_rp1", ";Count;R_{p} EM1;"     , 100, 0.0  , 1.00) );
  store->add( new TH1F("cl_rp2", ";Count;R_{p} EM2;"     , 100, 0.0  , 1.00) );
  store->add( new TH1F("cl_rp3", ";Count;R_{p} EM3;"     , 100, 0.0  , 1.00) );
  store->add( new TH1F("cl_rphad1", ";Count;R_{p} HAD1;" , 100, 0.0  , 1.00) );
  store->add( new TH1F("cl_rphad2", ";Count;R_{p} HAD2;" , 100, 0.0  , 1.00) );
  store->add( new TH1F("cl_rphad3", ";Count;R_{p} HAD3;" , 100, 0.0  , 1.00) );
...
StatusCode CaloClusterMaker::fillHistograms(EventContext &ctx ) const
{
  ...
  for( const auto& clus : **clusters.ptr() ){
    ...
    store->hist1("cl_rp0")->Fill( clus->rp0() );
    store->hist1("cl_rp1")->Fill( clus->rp1() );
    store->hist1("cl_rp2")->Fill( clus->rp2() );
    store->hist1("cl_rp3")->Fill( clus->rp3() );
    store->hist1("cl_rphad1")->Fill( clus->rphad1() );
    store->hist1("cl_rphad2")->Fill( clus->rphad2() );
    store->hist1("cl_rphad3")->Fill( clus->rphad3() );
    ...
    MSG_DEBUG( "Rp0      : " << clus->rp0()    );
    MSG_DEBUG( "Rp1      : " << clus->rp1()    );
    MSG_DEBUG( "Rp2      : " << clus->rp2()    );
    MSG_DEBUG( "Rp3      : " << clus->rp3()    );
    MSG_DEBUG( "Rphad1   : " << clus->rphad1() );
    MSG_DEBUG( "Rphad2   : " << clus->rphad2() );
    MSG_DEBUG( "Rphad3   : " << clus->rphad3() );
